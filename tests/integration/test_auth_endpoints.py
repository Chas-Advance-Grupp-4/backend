...
def test_login_user_success(client, admin_headers_fixture, new_user_data):
    """
    ...
    response = client.post("/api/v1/auth/login", json={"username": new_user_data["username"], "password": new_user_data["password"]})
...
def test_login_user_wrong_password(new_user_data):
    ...
    response = client.post("/api/v1/auth/login", json={"username": new_user_data["username"], "password": "wrongpass"})
